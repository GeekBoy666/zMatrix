...
namespace z {
    ...
    template<class _type>
    inline void _Matrix<_type>::create(int _rows, int _cols) {
    ...
    template<class _type>
    _Matrix<_type>::_Matrix(_Size<int> size) {
    ...
    template<class _type>
    _Matrix<_type>::_Matrix(_Size<int> size, int _chs) {
        _log_("Matrix construct with params.");
        initEmpty();
     ...
     */
    template<class _type>
    _Matrix<_type>::_Matrix(int _rows, int _cols) {
        _log_("Matrix construct with params.");
        initEmpty();
        create(_rows, _cols);
    }
    ...
    template<class _type>
    _Matrix<_type>::_Matrix(int _rows, int _cols, int _chs) {
        _log_("Matrix construct with params.");
        initEmpty();
     ...
     */
    template<class _type>
    _Matrix<_type>::_Matrix(const _Matrix <_type> &m)
    ...
    template<class _type>
    template<class _Tp2>
    _Matrix<_type>::operator _Matrix<_Tp2>() const {
...
#if defined(OPENCV)
#define  CONVERSION(param) do{cv::Mat_<cv::Vec<_type, param>> temp(rows, cols, 1);\
...
#undef CONVERSION
     ...
     */
    template<class _type>
    inline _type *_Matrix<_type>::ptr(int i0) {
    ...
    template<class _type>
    inline const _type *_Matrix<_type>::ptr(int i0) const {
        if ((unsigned) i0 >= (unsigned) _size) {
            return nullptr;
        }
        return data + i0 * step;
    ...
    /**
     * @berif 返回(i0, i1)像素的头指针
     */
    template<class _type>
    inline _type *_Matrix<_type>::ptr(int i0, int i1) {
    ...
    template<class _type>
    inline const _type *_Matrix<_type>::ptr(int i0, int i1) const {
        if ((unsigned) i0 >= (unsigned) rows || (unsigned) i1 >= (unsigned) cols) {
            return nullptr;
        }
        return data + i0 * step + i1 * chs;
    ...
    template<class _type>
    inline int _Matrix<_type>::swap(int32_t i0, int32_t j0, int32_t i1, int32_t j1) {
        _type temp;
        ...
        //ptr(i0, j0)[k] ^= ptr(i1, j1)[k] ^= ptr(i0, j0)[k] ^= ptr(i1, j1)[k];
    }
     ...
     */
    template<class _type>
    void _Matrix<_type>::conv(Matrix &kernel, _Matrix <_type> &dst, bool norm) {
    ...
    template<class _type>
    void conv(_Matrix <_type> &src, _Matrix <_type> &dst, Matrix &core) {
        src.conv(core, dst);
    }
